...
let d = app.election.create('What will we eat for dinner?', ['pizza', 'sushi', 'hotdog', 'nothing'], Date.now() / 1000 + 5 * 60 * 60);
...
app.election.voteProcess(peer1, peer1.signVote({ answer: 'pizza', id: d.getId() }));
app.election.voteProcess(peer2, peer2.signVote({ answer: 'pizza', id: d.getId() }));
app.election.voteProcess(peer3, peer3.signVote({ answer: 'hotdog', id: d.getId() }));
app.election.voteProcess(peer4, peer4.signVote({ answer: 'sushi', id: d.getId() }));

console.log(app.dataManager.getData(d.getId()).getBalanced());
